...
| `regression_analysis.ipynb` | **The RQ1 + RQ2 analysis as a notebook, already executed** — every cell's output (model summaries, R², VIF tables, the moderation comparison table) is baked in, so it can be read top-to-bottom without re-running anything. Imports `hedonic_pricing_model.py` and `moderation_analysis.py` directly rather than duplicating their logic, so the notebook can't drift out of sync with the committed scripts. This is the file to open for the actual R²/VIF/AIC numbers, or to screenshot for the write-up. |
